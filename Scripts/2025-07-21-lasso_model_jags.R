model{
  ...
  for(covar in 1:ncovar){
    ...
    laplace[covar] ~ ddexp(0, lambda) # lasso regression
    pi[covar] ~ dbern(pp) # indicator variable
    psi_beta[covar] <- laplace[covar] * pi[covar] # mixture of priors
  }
  
  # hyper prior for indicator variable
  pp ~ dbeta(3,2)
